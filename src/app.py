...
def post_member():
    ...
    if body is None:
        return jsonify({"msg": "Debes enviar información del body"}), 400
    if "first_name" not in body:
        return jsonify({'msg': "El campo first_name es obligatorio"}), 400
    if "age" not in body:
        return jsonify({'msg': "El campo age es obligatorio"}), 400
    if "lucky_numbers" not in body:
        return jsonify({'msg': "El campo lucky_numers es obligatorio"}), 400
    new_member = {
             'id': jackson_family._generateId(),
             'first_name':body['first_name'],
             'last_name':jackson_family.last_name,
             'age':body['age'],
             'luky':body['lucky_numbers'],
            }
...
def delete_member(id):
    ...
    if member:
        ...
        return jsonify({'msg':f"Member deleted:${member}"}), 200
    else:
